# AIRchetipo Installation & Module System Reference
...
AIRchetipo Core is a modular AI agent framework with intelligent installation, platform-agnostic support, and configuration inheritance.

### Key Features

- **Modular Design**: Core + optional modules (AIB, AIM)
- **Smart Installation**: Interactive configuration with dependency resolution
- **Clean Architecture**: Centralized `{air_folder}` directory add to project, no source pollution with multiple folders added
...
```
project-root/
├── {air_folder}/             # Centralized installation
│   ├── _cfg/                  # Configuration
...
AIRchetipo for software development workflows
...
2. **Content Injection** (`sub-modules/{platform}/injections.yaml`)
   ...
   ```yaml
   injections:
     - file: '{air_folder}/aim/agents/pm.md'
...
Generated in: `air/_cfg/agents/{module}-{agent}.md`
...
| Issue                   | Solution                                   |
| ----------------------- | ------------------------------------------ |
| Existing installation   | Use `air update` or remove `{air_folder}/` |
| Module not found        | Check `src/modules/` exists                |
| Config not applied      | Verify `{air_folder}/{module}/config.yaml` |
...
```bash
air install -v     # Verbose installation
air status -v      # Detailed status
```

### Best Practices

1. Run from project root
2. Backup `{air_folder}/_cfg/` before updates
...
| v4                  | v6                          |
| ------------------- | --------------------------- |
| Scattered files     | Centralized `{air_folder}/` |
| Monolithic          | Modular                     |
...
menu:
  - trigger: create-story
    workflow: '{project-root}/{air_folder}/aim/workflows/4-implementation/create-story/workflow.yaml'
    workflow-install: '{project-root}/{air_folder}/custom-module/workflows/create-story/workflow.yaml'
    description: 'Create a feature story'
```
...
3. **Config Update**: Vendored workflows get their `config_source` updated:
   ...
   ```yaml
   # Source workflow (in aim):
   config_source: "{project-root}/{air_folder}/aim/config.yaml"

   # Vendored workflow (in custom-module):
   config_source: "{project-root}/{air_folder}/custom-module/config.yaml"
   ```
...
```bash
air bundle --web           # Filter for web deployment
