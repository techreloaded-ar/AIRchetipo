...
Customize AIRchetipo agents without modifying core files. All customizations persist through updates.
...
```bash
npx airchetipo@alpha install # and then select option to compile all agents
# OR for individual agent only
npx airchetipo@alpha build <agent-name>
...
```yaml
# {air_folder}/_cfg/agents/aim-dev.customize.yaml
agent:
  metadata:
    name: 'TDD Developer'
...
menu:
  - trigger: deploy-staging
    workflow: '{project-root}/.air-custom/deploy-staging.yaml'
    description: Deploy to staging environment
  - trigger: deploy-prod
    workflow: '{project-root}/.air-custom/deploy-prod.yaml'
...
```yaml
# {air_folder}/_cfg/agents/aim-pm.customize.yaml
persona:
...
- **Update-Safe:** Your customizations in `_cfg/` survive all AIRchetipo updates
...
- Customize agents per-project in `{air_folder}/_cfg/agents/`
...
- Make sure you ran `npx airchetipo build <agent-name>` after editing
...
- Run `npx airchetipo build <agent-name>` to regenerate defaults

## Next Steps

- **[AIM Agents Guide](../src/modules/aim/docs/agents-guide.md)** - Learn about all 12 AIRchetipo agents
- **[AIB Create Agent Workflow](../src/modules/aib/workflows/create-agent/README.md)** - Build completely custom agents
- **[AIM Complete Documentation](../src/modules/aim/docs/README.md)** - Full AIRchetipo reference
