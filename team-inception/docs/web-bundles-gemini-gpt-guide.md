# Using AIRchetipo Web Bundles in Gemini Gems & Custom GPTs

Web bundles package AIRchetipo agents as self-contained XML files that work in Gemini Gems and Custom GPTs. Everything the agent needs - instructions, workflows, dependencies - is bundled into a single file.
...
**Perfect for:** Uploading a single file to a Gemini GEM or Custom GPT to use AIRchetipo from the Web, generally at a huge cost savings, at the expense of some quality and convenience of using locally.

## Critical Setup Rules

**READ THIS FIRST - Following these rules ensures AIRchetipo works correctly in Gemini/GPT:**
...
4. **Gemini Gems are strongly preferred** - They work significantly better than Custom GPTs for AIRchetipo workflows.
...
**[→ Browse Web Bundles on GitHub](https://github.com/airchetipo-org/AIRchetipo/tree/main/web-bundles)**

Navigate to the module folder (aim, aib) → agents folder → download the `.xml` file you need.

**Option B: Generate from Local Installation**

From your AIRchetipo project directory:
...
node tools/cli/bundlers/bundle-web.js module aim
node tools/cli/bundlers/bundle-web.js agent aim dev
```
...
3. Give your Gem a name (e.g., "AIRchetipo PM Agent")
...
- aim-workflow-status: standalone (no workflow)
```
...
3. Configure your GPT:
   - **Name:** AIRchetipo PM Agent (or your choice)
   - **Description:** AI planning agent powered by AIRchetipo
...
### AIRchetipo Builder (AIB) Agent

- **air-builder.xml** - Create custom agents, workflows, and modules
...
- **aim/teams/team-fullstack.xml** - Full AIRchetipo development team
...
2. Run local AIRchetipo installation with `*workflow-init`
3. AIRchetipo will detect the existing artifacts and update workflow status
...
- Works excellently with AIRchetipo workflows
...
1. Edit `{air_folder}/_cfg/agents/<agent>.customize.yaml`
2. Rebuild: `npx airchetipo build <agent-name>`
...
2. **Prefer Gemini Over GPT** - Gemini Gems work significantly better with AIRchetipo bundles
...
10. **Keep Updated** - Rebuild bundles after AIRchetipo updates to get latest improvements
...
1. **Download bundles:**
   - `aim/agents/analyst.xml`
   - `aim/agents/pm.xml`
   - `aim/agents/architect.xml`
   - `aim/agents/ux-designer.xml`
...
7. **Setup local AIRchetipo:**
   - Install AIRchetipo locally: `npx airchetipo@alpha install`
   - Place exported docs in project `docs/` folder
   - Load Developer agent
   - Run: `*workflow-init` → AIRchetipo detects artifacts, suggests next steps
...
1. Download `aim/agents/architect.xml`
...
- **[AIM Documentation](../src/modules/aim/docs/README.md)** - Learn all workflows
...
- **[AIRchetipo Discord](https://discord.gg/gk8jAdXWmj)** - Get help and share your Gems/GPTs
