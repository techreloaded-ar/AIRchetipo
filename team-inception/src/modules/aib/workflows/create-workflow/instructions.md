...
<critical>You MUST have already loaded and processed: {project-root}/{air_folder}/aib/workflows/create-workflow/workflow.yaml</critical>
...
- Workflow: {project-root}/{air_folder}/core/workflows/brainstorming/workflow.yaml
...
- What module will it belong to? (e.g., "aim", "aib", "cis")
...
- Author name (default to user_name or "AIRchetipo")
...
config_source: '{project-root}/{air_folder}/{{target_module}}/config.yaml'
...
# Critical variables from config
config_source: '{project-root}/{air_folder}/module/config.yaml'
output_folder: '{config_source}:output_folder'
user_name: '{config_source}:user_name'
communication_language: '{config_source}:communication_language'
date: system-generated
...
1. ALWAYS include critical headers:
   - Workflow engine reference: {project-root}/{air_folder}/core/tasks/workflow.xml
...
4. Use proper XML tags from guide:
   ...
   - Output: <template-output>, <invoke-task halt="true">{project-root}/{air_folder}/core/tasks/advanced-elicitation.xml</invoke-task>, <critical>, <example>
...
- File paths use {air_folder}/ root (not {project-root})

<action>Configure web_bundle section in workflow.yaml:</action>

1. Copy core workflow metadata (name, description, author)
2. Convert all file paths to {air_folder}/-relative paths:
   - Remove {project-root}/ prefix
   - Remove {config_source} references (use hardcoded values)
   - Example: "{project-root}/{air_folder}/aim/workflows/x" → "{air_folder}/aim/workflows/x"
...
3. List ALL referenced files by scanning:
   ...
   **Critical: Workflow Dependencies**
   ...
   - Example: `<invoke-workflow>{project-root}/{air_folder}/core/workflows/x/workflow.yaml</invoke-workflow>`
     → Add "{air_folder}/core/workflows/x/workflow.yaml" to web_bundle_files
...
web_bundle:
  ...
  existing_workflows:
    - workflow_variable_name: '{air_folder}/path/to/workflow.yaml'
```
...
web_bundle:
  ...
  author: 'AIRchetipo'
  instructions: '{air_folder}/aim/workflows/brainstorm-game/instructions.md'
  template: false
  web_bundle_files:
    - '{air_folder}/aim/workflows/brainstorm-game/instructions.md'
    - '{air_folder}/mmm/workflows/brainstorm-game/game-context.md'
    - '{air_folder}/core/workflows/brainstorming/workflow.yaml'
  existing_workflows:
    - core_brainstorming: '{air_folder}/core/workflows/brainstorming/workflow.yaml'
```
...
- Next steps:
  - Run the AIRchetipo Method installer to this project location
