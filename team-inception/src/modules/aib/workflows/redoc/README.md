...
**Module:** AIRchetipo Builder (aib)

## Purpose

ReDoc is an intelligent documentation maintenance system for AIRchetipo modules, workflows, and agents. It uses a reverse-tree approach (leaf folders first, then parent folders) to systematically generate and update README.md files with technical writer quality output.

The workflow understands AIRchetipo conventions deeply and focuses documentation on distinctive features rather than explaining standard patterns, resulting in succinct, precise technical documentation.
...
- **Convention-Aware**: Loads AIRchetipo architecture patterns and only documents unique/distinctive aspects
...
- **Module path**: `{air_folder}/aim` (documents entire module: root, workflows, agents)
- **Workflows folder**: `{air_folder}/aim/workflows` (documents all workflows)
- **Agents folder**: `{air_folder}/aim/agents` (documents all agents)
- **Single workflow**: `{air_folder}/aim/workflows/product-brief` (documents one workflow)
- **Single agent**: `{air_folder}/aim/agents/prd-agent.md` (documents one agent)
...
1. **Initialize**: Load AIRchetipo conventions and validate target
...
- **Context-Aware**: Uses AIRchetipo convention knowledge to highlight only distinctive features
