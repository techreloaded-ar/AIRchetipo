...
The Convert Legacy workflow is a comprehensive migration tool that converts AIRchetipo v4 items (agents, workflows, modules) to v6 compliant format with proper structure and conventions. It bridges the gap between legacy AIRchetipo implementations and the modern v6 architecture, ensuring seamless migration while preserving functionality and improving structure.
...
The workflow uses standard AIB configuration:
...
- Maps `elicit: true` flags to `<invoke-task halt="true">{project-root}/{air_folder}/core/tasks/advanced-elicitation.xml</invoke-task>` tags
...
- **Solution**: Ensure all references use `{project-root}/{air_folder}/` format
...
- Review the workflow creation guide at `/{air_folder}/aib/workflows/create-workflow/workflow-creation-guide.md`
- Check conversion mappings at `/{air_folder}/aib/data/v4-to-v6-mappings.yaml`
- Validate output using `checklist.md`
- Consult AIRchetipo v6 documentation for proper conventions

---

_Part of the AIRchetipo v6 - AIB (Builder) Module_
