...
<parameter name="You MUST have already loaded and processed: {project-root}/{air_folder}/aib/workflows/convert-legacy/workflow.yaml</critical>
...
<ask>Which module should this belong to? (eg. aim, aib, cis, aim-legacy, or custom)</ask>
...
<critical>IMPORTANT: All paths must use final AIRchetipo installation locations, not src paths!</critical>
<action>Show user the target location: {project-root}/{air_folder}/{{target_module}}/{{item_type}}/{{item_name}}</action>
<action>Note: Files will be created in {air_folder}/ but all internal paths will reference {project-root}/{air_folder}/ locations</action>
...
5. Handle dependencies and paths:
   ...
   - CRITICAL: All paths must use {project-root}/{air_folder}/{{module}}/ NOT src/

<action>Generate the converted v6 agent YAML file (.agent.yaml)</action>
<action>Example path conversions:

- exec="{project-root}/{air_folder}/{{target_module}}/tasks/task-name.md"
- run-workflow="{project-root}/{air_folder}/{{target_module}}/workflows/workflow-name/workflow.yaml"
- data="{project-root}/{air_folder}/{{target_module}}/data/data-file.yaml"
  </action>
  <action>Save to: {air_folder}/{{target_module}}/agents/{{agent_name}}.agent.yaml (physical location)</action>
...
<invoke-workflow>
  workflow: {project-root}/{air_folder}/aib/workflows/create-agent/workflow.yaml
  inputs:
...
2. Convert template sections to instructions.md:
   ...
   - `elicit: true` → `<invoke-task halt="true">{project-root}/{air_folder}/core/tasks/advanced-elicitation.xml</invoke-task>` tag
...
<invoke-workflow>
  workflow: {project-root}/{air_folder}/aib/workflows/create-workflow/workflow.yaml
  inputs:
...
<invoke-workflow>
  workflow: {project-root}/{air_folder}/aib/workflows/create-module/workflow.yaml
  inputs:
...
4. Handle special v4 patterns:
   - 1-9 elicitation menus → v6 <invoke-task halt="true">{project-root}/{air_folder}/core/tasks/advanced-elicitation.xml</invoke-task>
   - Agent permissions → note in instructions
