...
<critical>You MUST have already loaded and processed: {project-root}/{air_folder}/aib/workflows/create-agent/workflow.yaml</critical>
<critical>Study YAML agent examples in: {project-root}/{air_folder}/aim/agents/ for patterns</critical>
...
<step n="-1" goal="Optional brainstorming for agent ideas" optional="true">
  ...
  <check if="user answered yes">
    <action>Invoke brainstorming workflow: {project-root}/{air_folder}/core/workflows/brainstorming/workflow.yaml</action>
...
**Path Determination:**
  ...
  <check if="module agent selected">
    <action>Discover which module system fits best (aim, aib, cis, or custom)</action>
    <action>Store as {{target_module}} for path determination</action>
    <note>Agent will be saved to: {air_folder}/{{target_module}}/agents/</note>
  </check>

  <check if="standalone agent selected">
    <action>Explain this will be their personal agent, not tied to a module</action>
    <note>Agent will be saved to: {air_folder}/agents/{{agent-name}}/</note>
    <note>All sidecar files will be in the same folder</note>
  </check>

<critical>Determine agent location:</critical>

- Module Agent → {air_folder}/{{module}}/agents/{{agent-name}}.agent.yaml
- Standalone Agent → {air_folder}/agents/{{agent-name}}/{{agent-name}}.agent.yaml
...
Explain: "Most AIRchetipo v6 workflows default to **intent-based + medium/high interactivity**
...
- trigger: [another capability]
  workflow: "{project-root}/{air_folder}/SOURCE_MODULE/workflows/path/to/workflow.yaml"
  workflow-install: "{project-root}/{air_folder}/THIS_MODULE/workflows/vendored/path/workflow.yaml"
...
<example type="yaml">
  agent:
    metadata:
      id: {air_folder}/{{target_module}}/agents/{{agent_filename}}.md
...
<step n="8b" goal="Handle build tools availability">
  <action>Check if AIRchetipo build tools are available in this project</action>

  <check if="AIRchetipo project with build tools">
...
1. Run the AIRchetipo Method installer to this project location
