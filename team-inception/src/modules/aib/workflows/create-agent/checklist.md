...
- [ ] `metadata.id` points to final compiled path: `{air_folder}/{{module}}/agents/{{agent}}.md`
- [ ] `metadata.module` matches the module folder (e.g., `aim`, `aib`, `cis`)
...
- [ ] Customize file (if created) located at `{project-root}/{air_folder}/_cfg/agents/{{module}}-{{agent}}.customize.yaml`

## Build Verification

- [ ] Run compile to build `.md`: `npm run install:air` → "Compile Agents" (or `air install` → Compile)
- [ ] Confirm compiled file exists at `{project-root}/{air_folder}/{{module}}/agents/{{agent}}.md`
