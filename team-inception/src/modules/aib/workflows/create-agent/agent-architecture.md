# AIRchetipo Agent Architecture Reference
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/{air_folder}/{module}/config.yaml and set variables</i>
...
```xml
<module-integration>
  <module-path>{project-root}/{air_folder}/{module-code}</module-path>
  <config-source>{module-path}/config.yaml</config-source>
  <workflows-path>{project-root}/{air_folder}/{module-code}/workflows</workflows-path>
</module-integration>
```
...
```
Good: {project-root}/{air_folder}/{module}/agents/
...
```xml
<!-- Full path -->
<item cmd="*create-prd" run-workflow="{project-root}/{air_folder}/aim/workflows/prd/workflow.yaml">
...
```xml
<item cmd="*validate" exec="{project-root}/{air_folder}/core/tasks/validate-workflow.xml">
  Validate document
</item>
```
...
```xml
<item cmd="*standup"
   exec="{project-root}/{air_folder}/aim/tasks/daily-standup.xml"
...
<item cmd="*run" exec="{project-root}/{air_folder}/module/task.md">
...
**Workflow Interaction Styles** (AIRchetipo v6 default):
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/{air_folder}/{module}/config.yaml</i>
  <i>Remember the users name is {user_name}</i>
  <i>ALWAYS communicate in {communication_language}</i>
</critical-actions>
```
...
```xml
<agent id="{air_folder}/{module}/agents/{name}.md"
       name="{Name}"
       title="{Title}"
       icon="{emoji}">
  <persona>...</persona>
