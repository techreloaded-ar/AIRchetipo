 ...
 * Called by AIRCHETIPO installer when processing the module
 */
...
async function setupConfigurations(config) {
  ...
  const configPath = path.join(config.project_root, 'air', config.module_code, 'config.json');
...
// Export functions for AIRCHETIPO installer
module.exports = {
  installModule,
  initDatabase,
  generateSamples,
