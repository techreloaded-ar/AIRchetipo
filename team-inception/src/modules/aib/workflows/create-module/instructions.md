...
<critical>You MUST have already loaded and processed: {project-root}/{air_folder}/aib/workflows/create-module/workflow.yaml</critical>
<critical>Study existing modules in: {project-root}/{air_folder}/ for patterns</critical>
...
<check if="create">
  <action>Invoke module-brief workflow: {project-root}/{air_folder}/aib/workflows/module-brief/workflow.yaml</action>
...
2. **Module code** - Generate kebab-case from name following patterns:
   ...
   - Present suggested code and confirm it works for paths like {air_folder}/{{code}}/agents/
...
<critical>This is the STANDARD format that AIRchetipo installer uses</critical>
...
# EXAMPLE Static path:
# data_path:
#   result: "{project-root}/{air_folder}/{{module_code}}/data"
...
The module can be configured in `{air_folder}/{{module_code}}/config.yaml`
