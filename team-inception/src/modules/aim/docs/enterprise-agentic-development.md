# Enterprise Agentic Development with AIRchetipo
...
- [How AIRchetipo Enables PM/UX Technical Evolution](#how-airchetipo-enables-pmux-technical-evolution)
...
**Impact:** Single developer with AIRchetipo can deliver in 1 day what previously required full team and multiple sprints.
...
**The Bottom Line:** By 2026, successful PMs/UX will fluently operate in both vision and execution. **AIRchetipo provides the structured framework to make this transition.**

---

## How AIRchetipo Enables PM/UX Technical Evolution

AIRchetipo is specifically designed to position PMs and UX designers for this future.
...
**Why it matters:** Traditional PRDs are human-readable prose. AIRchetipo PRDs are **AI-executable work packages**.
...
```bash
air pm *create-epics-and-stories
```
...
**PM Value:** No more "story refinement sessions" with engineering. AI agents execute directly from AIRchetipo stories.
...
```bash
air architect *create-architecture
```
...
```
PM writes AIRchetipo PRD
   ↓
create-epics-and-stories generates story queue
   ↓
Stories loaded by human developers + AIRchetipo agents
   ↓
...
- **AIRchetipo Agentic:** PM writes PRD → AI agents implement → review PRs → deploy (2-5 days)
...
**UX Value:** Designs validated through working prototypes, not static mocks. Technical understanding built through AIRchetipo workflows.

### 6. PM Technical Skills Development

**AIRchetipo teaches PMs technical skills through:**
...
- AIRchetipo asks about scale, performance, integrations
- PM answers business questions
- AIRchetipo explains technical implications
...
**AIRchetipo Agentic Model:**

- 1 PM → writes AIRchetipo PRD → 20-50 AI agents execute stories in parallel → delivers 5-10 features/quarter
...
**PM Workflow (with AIRchetipo + Cursor/v0):**

1. Use AIRchetipo to generate PRD structure and requirements
2. Extract key user flow from PRD
3. Feed to Cursor/v0 with AIRchetipo context
...
**AIRchetipo Agentic:** Hours of development to validate idea

### 10. Career Path Evolution

**AIRchetipo positions PMs for emerging roles:**
...
**Hiring advantage:** PMs using AIRchetipo demonstrate:
...
**Problem:** Teams customize AIRchetipo (agents, workflows, configs) but don't want personal tooling in main repo.

**Anti-pattern:** Adding `{air_folder}/` to `.gitignore` breaks IDE tools, submodule management.

### The Solution: Git Submodules

**Benefits:**

- AIRchetipo exists in project but tracked separately
- Each developer controls their own AIRchetipo version/config
...
git submodule add https://github.com/your-org/aim-config.git air
git commit -m "Add AIM as submodule"
```
...
# Make personal customizations in {air_folder}/
```
...
```bash
cd /path/to/your-project
# AIRchetipo available at ./{air_folder}/, load agents normally
```
...
**AIM Setup:** Direct installation, both use Claude Code, minimal customization
...
**AIM Setup:** Git submodule, team config repo, mix of Claude Code/Cursor users
...
**AIM Setup:** Each team has own submodule config, org-wide base config, variety of IDE tools
...
**Git submodules:** Best practice for AIM management across teams
**Team flexibility:** Mix of tools (Claude Code, Cursor, Windsurf) with shared AIM foundation
**Scalable patterns:** Epic-based, layer-based, feature-based distribution strategies

### The Future (2026)

PMs write AIRchetipo PRDs → Stories auto-fed to cloud AI agents → Parallel implementation → Human review of PRs → Continuous deployment
...
- [Agents Guide](./agents-guide.md) - Understanding AIRchetipo agents

---

_AIRchetipo fundamentally changes how PMs work, how teams structure work, and how products get built. Understanding these patterns is essential for enterprise success in the age of AI agents._
