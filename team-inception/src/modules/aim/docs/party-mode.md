...
Type `/air:core:workflows:party-mode` (or `*party-mode` from any agent), and suddenly you've got **all your AI agents** in one conversation. PM, Architect, DEV, UX Designer, Technical Writer, and more - everyone shows up.
...
**PM (John):** "What if onboarding was a story? Each step reveals a character's journey - the user IS the hero."

**Game Designer:** "Take it further - gamify with unlockable achievements. But make them meaningful, not arbitrary badges."

**Architect (Winston):** "Building on that - what if the first 'quest' is actually solving a real user problem? They learn by doing something valuable."
...
Party mode uses agents from `{air_folder}/[module]/agents/*.md` - these already include any customizations you applied during install.

**To customize agents for party mode:**

1. Create customization file: `{air_folder}/_cfg/agents/aim-pm.customize.yaml`
2. Run `npx airchetipo install` to rebuild agents
...
## AIM Workflows That Use Party Mode
