# AIM Planning Workflows (Phase 2)
...
graph TB
    ...
    subgraph AIRchetipoMethod["<b>AIRchetipo METHOD (Recommended)</b>"]
        direction TB
    ...
    subgraph Enterprise["<b>ENTERPRISE METHOD</b>"]
        direction TB
        EntNote["<b>Uses AIRchetipo Planning</b><br/>+<br/>Extended Phase 3 workflows<br/>(Architecture + Security + DevOps)<br/>30+ stories typically"]
    end
    ...
    Enterprise -.->|Uses AIRchetipo planning| Epics
    ...
    style AIRchetipoMethod fill:#e1bee7,stroke:#6a1b9a,stroke-width:3px,color:#000
...
| **narrative**                | Game Designer | AIRchetipo | Story-driven game/experience design        | 10-50+          |
| **create-epics-and-stories** | PM            | AIRchetipo | Break PRD/GDD into Epic+Stories            | N/A             |
| **ux**                       | UX Designer   | AIRchetipo | Optional UX specification                  | N/A             |
...
AIM uses three distinct planning tracks that adapt to project complexity:
...
### Track 2: AIRchetipo (RECOMMENDED)
...
**Planning (Phase 2):** Uses AIRchetipo planning (PRD + Epic+Stories)
...
**Purpose:** Complete game design document for game projects (AIRchetipo track).
...
**Purpose:** Story-driven design workflow for games/experiences where narrative is central (AIRchetipo track).
...
**Purpose:** UX specification for projects where user experience is the primary differentiator (AIRchetipo track).
...
**Purpose:** Break PRD/GDD requirements into bite-sized stories organized in epics (AIRchetipo track).
...
- **Software product** → `prd` (AIRchetipo)
- **Game (gameplay-first)** → `gdd` (AIRchetipo)
- **Game (story-first)** → `narrative` + `gdd` (AIRchetipo)
- **UX innovation project** → `ux` + `prd` (AIRchetipo)
...
| PRD.md              | **architecture** (Level 3-4)         | AIRchetipo (recommended)     |
| GDD.md              | **architecture** (game tech)         | AIRchetipo (recommended)     |
| narrative-design.md | **architecture** (narrative systems) | AIRchetipo                   |
| ux-spec.md          | **architecture** (frontend design)   | AIRchetipo                   |
...
- **AIRchetipo:** Optional Phase 3 (simple), Required Phase 3 (complex)
...
If `workflow-init` suggests AIRchetipo, there's likely complexity you haven't considered. Review carefully before overriding.
...
### Greenfield Software (AIRchetipo)
...
A: No. Simple changes use `tech-spec` (Quick Flow). Only AIRchetipo and Enterprise tracks create PRDs.

**Q: Can I skip Phase 3 (Solutioning)?**
A: Yes for Quick Flow. Optional for AIRchetipo (simple projects). Required for AIRchetipo (complex projects) and Enterprise.
