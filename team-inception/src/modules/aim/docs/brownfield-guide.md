# AIRchetipo Brownfield Development Guide
...
| **AIRchetipo**        | Feature sets, integrations | 10-50+          | Integrate with existing architecture            |
...
- Keywords: "fix", "bug" → Quick Flow, "dashboard", "platform" → AIRchetipo, "enterprise", "multi-tenant" → Enterprise Method
...
```
System: "Found PRD.md (AIRchetipo track, 30 stories, 6 months old)"
...
2. **Then:** Run `index-docs` task to create navigation:
   ...
   ```bash
   {air_folder}/core/tasks/index-docs.xml --directory ./docs
   ```
...
{air_folder}/core/tools/shard-doc.xml \
...
- **PRD** (AIRchetipo) can't reference existing code → Designs incompatible features
...
- `product-brief` - Strategic planning (AIRchetipo/Enterprise tracks only)
...
See the [Workflows section in AIM README](../README.md) for details.
...
**AIRchetipo/Enterprise:** Use `prd` workflow
...
### Phase 3: Solutioning (AIRchetipo/Enterprise Only)
...
- `create-architecture` - Extend architecture docs (AIRchetipo/Enterprise)
- `solutioning-gate-check` - Validate before implementation (AIRchetipo/Enterprise)
...
### Scenario 3: Feature Set (AIRchetipo)
...
### Scenario 4: Complex Integration (AIRchetipo)
...
prd                     # AIRchetipo/Enterprise tracks

# Phase 3: Solutioning (AIRchetipo/Enterprise)
# Architect agent:
...
- `docs/aim-workflow-status.yaml` - Progress tracker

**Phase 2 Planning:**

- `docs/tech-spec.md` (Quick Flow track)
- `docs/PRD.md` (AIRchetipo/Enterprise tracks)
- Epic breakdown

**Phase 3 Architecture:**

- `docs/architecture.md` (AIRchetipo/Enterprise tracks)
...
flowchart TD
    ...
    TRACK -->|AIRchetipo| PRD[prd → architecture]
...
5. ✅ **Start with simpler track if unsure** - Easy to upgrade (Quick Flow → AIRchetipo)
...
- **[Quick Start Guide](./quick-start.md)** - Getting started with AIM
...
- [GitHub Issues](https://github.com/airchetipo-org/AIRchetipo/issues)
- [YouTube Channel](https://www.youtube.com/@AIRchetipoCode)

**Documentation:**

- [Test Architect Guide](./test-architecture.md) - Comprehensive testing strategy
- [AIM Module README](../README.md) - Complete module and workflow reference
