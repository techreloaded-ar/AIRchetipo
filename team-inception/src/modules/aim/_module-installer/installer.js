...
async function install(options) {
  ...
  try {
    logger.log(chalk.blue('🚀 Installing AIM Module...'));
    ...
    if (installedIDEs && installedIDEs.length > 0) {
      logger.log(chalk.cyan(`Configuring AIM for IDEs: ${installedIDEs.join(', ')}`));

      // Add any IDE-specific AIM configurations here
      for (const ide of installedIDEs) {
        await configureForIDE(ide, projectRoot, config, logger);
      }
    }

    logger.log(chalk.green('✓ AIM Module installation complete'));
    return true;
  } catch (error) {
    logger.error(chalk.red(`Error installing AIM module: ${error.message}`));
    return false;
  }
}

/**
 * Configure AIM module for specific platform/IDE
 * @private
 */
async function configureForIDE(ide, projectRoot, config, logger) {
  // Validate platform code
  if (!platformCodes.isValidPlatform(ide)) {
    logger.warn(chalk.yellow(`  Warning: Unknown platform code '${ide}'. Skipping AIM configuration.`));
    return;
  }
  ...
  try {
    if (await fs.pathExists(platformSpecificPath)) {
      ...
      logger.log(chalk.dim(`  No AIM-specific configuration for ${platformName}`));
    }
  } catch (error) {
    logger.warn(chalk.yellow(`  Warning: Could not load AIM platform-specific handler for ${platformName}: ${error.message}`));
  }
}
