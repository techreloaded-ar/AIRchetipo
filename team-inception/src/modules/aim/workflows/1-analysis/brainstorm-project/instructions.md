...
<critical>This is a meta-workflow that orchestrates the core brainstorming workflow with project-specific context</critical>

<workflow>

  <step n="1" goal="Validate workflow readiness" tag="workflow-status">
    <action>Check if {output_folder}/aim-workflow-status.yaml exists</action>
    ...
    <check if="status file found">
      <action>Load the FULL file: {output_folder}/aim-workflow-status.yaml</action>
  ...
  <step n="3" goal="Invoke core brainstorming with project context">
    <action>Execute the core brainstorming workflow with project context</action>
    <invoke-workflow path="{core_brainstorming}" data="{project_context}">
      The core brainstorming workflow will:
  ...
  <step n="4" goal="Update status and complete" tag="workflow-status">
    <check if="standalone_mode != true">
      ...
      <action>Update workflow_status["brainstorm-project"] = "{output_folder}/aim-brainstorming-session-{{date}}.md"</action>
...
- Brainstorming results saved to: {output_folder}/aim-brainstorming-session-{{date}}.md
