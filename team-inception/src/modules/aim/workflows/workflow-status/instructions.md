...
<critical>You MUST have already loaded and processed: {project-root}/{air_folder}/aim/workflows/workflow-status/workflow.yaml</critical>
...
<action>Search {output_folder}/ for file: aim-workflow-status.yaml</action>
...
<action>Read aim-workflow-status.yaml</action>
...
<check if="choice == 3">
  <action>Display complete aim-workflow-status.yaml file contents</action>
</check>
...
<action>Read {output_folder}/aim-workflow-status.yaml if exists</action>
...
<template-output>status_file_path = {{path to aim-workflow-status.yaml}}</template-output>
</check>
...
<action>Read {output_folder}/aim-workflow-status.yaml</action>
