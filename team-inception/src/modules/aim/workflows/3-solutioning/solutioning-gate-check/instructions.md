...
<critical>You MUST have already loaded and processed: {project-root}/{air_folder}/aim/workflows/3-solutioning/solutioning-gate-check/workflow.yaml</critical>
...
<check if="status file not found">
  <output>No workflow status file found. Implementation Ready Check can run standalone or as part of AIM workflow path.</output>
...
<check if="status file found">
  ...
  <action>Get {selected_track} (quick-flow, airchetipo, or enterprise-airchetipo)</action>
  <action>Find first non-completed workflow (next expected workflow)</action>

<action>Based on the selected_track, understand what artifacts should exist: - quick-flow: Tech spec and simple stories in an epic only (no PRD, minimal solutioning) - airchetipo and enterprise-airchetipo: PRD, tech spec, epics/stories, architecture, possible UX artifacts</action>
...
**Note:** test-design is recommended for AIRchetipo, required for Enterprise Method
...
<check if="standalone_mode != true">
  ...
  <action>Update workflow_status["solutioning-gate-check"] = "{output_folder}/aim-readiness-assessment-{{date}}.md"</action>
...
- Readiness assessment saved to: {output_folder}/aim-readiness-assessment-{{date}}.md
...
- Refer to the AIM workflow guide if unsure what to do next
