...
class UI {
  ...
  async promptInstall() {
    ...
    CLIUtils.displaySection('AIRchetipo Setup', `Agentic Product Development by Tech Reloaded v${version}`);

    const confirmedDirectory = await this.getConfirmedDirectory();

    // Preflight: Check for legacy AIRchetipo v4 footprints immediately after getting directory
    ...
    // Check if there's an existing AIRchetipo installation
    const fs = require('fs-extra');
    const path = require('node:path');
    // Use findAIRchetipoDir to detect any custom folder names (V6+)
    const airDir = await installer.findAIRchetipoDir(confirmedDirectory);
    const hasExistingInstall = await fs.pathExists(airDir);
      ...
      const promptResult = await inquirer.prompt([
        {
          ...
          choices: [
            ...
            { name: 'Modify AIRchetipo Installation (Confirm or change each setting)', value: 'update' },
            { name: 'Remove Air Folder and Reinstall (Full clean install - Air Customization Will Be Lost)', value: 'reinstall' },
  ...
  async promptToolSelection(projectDir, selectedModules) {
    // Check for existing configured IDEs - use findAIRchetipoDir to detect custom folder names
    const { Detector } = require('../installers/lib/core/detector');
    const { Installer } = require('../installers/lib/core/installer');
    const detector = new Detector();
    const installer = new Installer();
    const airDir = await installer.findAIRchetipoDir(projectDir || process.cwd());
  ...
  showInstallSummary(result) {
    CLIUtils.displaySection('Installation Complete', 'AIRchetipo has been successfully installed');
    ...
    console.log('\n' + chalk.green.bold('✨ AIRchetipo is ready to use!'));
  }
  ...
  async getExistingInstallation(directory) {
    ...
    const detector = new Detector();
    const installer = new Installer();
    const airDir = await installer.findAIRchetipoDir(directory);
    const existingInstall = await detector.detect(airDir);
  ...
  async selectModules(moduleChoices) {
    CLIUtils.displaySection('Module Selection', 'Choose the AIRchetipo modules to install');
  ...
  async displayDirectoryInfo(directory) {
    ...
    if (dirExists) {
      ...
      if (stats.isDirectory()) {
        ...
        if (files.length > 0) {
          // Check for any air installation (any folder with _cfg/manifest.yaml)
          const { Installer } = require('../installers/lib/core/installer');
          const installer = new Installer();
          const airDir = await installer.findAIRchetipoDir(directory);
          const hasAirInstall = (await fs.pathExists(airDir)) && (await fs.pathExists(path.join(airDir, '_cfg', 'manifest.yaml')));

          console.log(
            chalk.gray(`Directory exists and contains ${files.length} item(s)`) +
              (hasAirInstall ? chalk.yellow(` including existing AIRchetipo installation (${path.basename(airDir)})`) : ''),
          );
        } else {
