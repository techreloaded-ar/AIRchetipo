...
/**
 * Find the AIRCHETIPO project root directory by looking for package.json
 * or specific AIRCHETIPO markers
 */
function findProjectRoot(startPath = __dirname) {
  let currentPath = path.resolve(startPath);

  // Keep going up until we find package.json with airchetipo
  while (currentPath !== path.dirname(currentPath)) {
    ...
    if (fs.existsSync(packagePath)) {
      try {
        ...
        // Check if this is the AIRCHETIPO project
        if (pkg.name === 'airchetipo' || fs.existsSync(path.join(currentPath, 'src', 'core'))) {
          return currentPath;
        }
      } catch {
        // Continue searching
