...
const AgentPartyGenerator = {
  ...
  generateAgentParty(agentDetails, options = {}) {
    ...
    // Build XML content
    let xmlContent = `<!-- Powered by AIRchetipo-CORE -->
<!-- Agent Manifest - Generated during AIRchetipo ${forWeb ? 'bundling' : 'installation'} -->
<!-- This file contains a summary of all ${forWeb ? 'bundled' : 'installed'} agents for quick reference -->
<manifest id="air/_cfg/agent-manifest.csv" version="1.0" generated="${new Date().toISOString()}">
  <description>
    Complete roster of ${forWeb ? 'bundled' : 'installed'} AIRchetipo agents with summarized personas for efficient multi-agent orchestration.
...
`;
    ...
    for (const [module, agents] of Object.entries(agentsByModule)) {
      ...
      const moduleTitle =
        module === 'aim' ? 'AIM Module' : module === 'cis' ? 'CIS Module' : module === 'core' ? 'Core Module' : 'Custom Module';
  ...
  extractAgentDetails(content, moduleName, agentName) {
    try {
      ...
      return {
        id: `air/${moduleName}/agents/${agentName}.md`,
