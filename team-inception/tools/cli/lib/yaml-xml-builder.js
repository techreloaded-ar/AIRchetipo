...
class YamlXmlBuilder {
  ...
  async convertToXml(agentYaml, buildMetadata = {}) {
    ...
    if (buildMetadata.forWebBundle) {
      ...
      xml += '<!-- Powered by AIRchetipo-CORE -->\n\n';
      ...
      xml += `description: "${metadata.title || 'AIRchetipo Agent'}"\n`;
      xml += '---\n\n';
      xml +=
  ...
  async buildFromYaml(agentYamlPath, customizeYamlPath = null, options = {}) {
    ...
    // Extract module from path (e.g., /path/to/modules/aim/agents/pm.yaml -> aim)
    // or /path/to/air/aim/agents/pm.yaml -> aim
    ...
    const airIndex = pathParts.indexOf('air');

    if (modulesIndex !== -1 && pathParts[modulesIndex + 1]) {
      // Path contains /modules/{module}/
      module = pathParts[modulesIndex + 1];
    } else if (airIndex !== -1 && pathParts[airIndex + 1]) {
      // Path contains /air/{module}/
      const potentialModule = pathParts[airIndex + 1];
      // Check if it's a known module, not 'agents' or '_cfg'
      if (['aim', 'aib', 'cis', 'core'].includes(potentialModule)) {
