...
class WebBundler {
  constructor(sourceDir = null, outputDir = 'web-bundles') {
    ...
    this.tempManifestDir = path.join(this.tempDir, 'air', '_cfg');
  ...
  async bundleAgent(moduleName, agentFile, shouldTrack = true) {
    ...
    if (await fs.pathExists(defaultPartyPath)) {
      ...
      const wrappedParty = this.wrapContentInXml(processedPartyContent, 'air/_cfg/agent-manifest.csv', 'text');
      dependencies.set('air/_cfg/agent-manifest.csv', wrappedParty);
  ...
  async bundleTeam(moduleName, teamFile) {
    ...
    if (hasPartyFile) {
      // Load the party CSV and add it as air/_cfg/agent-manifest.csv
    ...
    // 1. First, always add the air-web-orchestrator (XML file only, no transformation needed)
    const orchestratorXmlPath = path.join(this.sourceDir, 'core', 'agents', 'air-web-orchestrator.agent.xml');
    ...
    if (await fs.pathExists(orchestratorXmlPath)) {
      ...
      console.log(chalk.gray(`    + Added orchestrator: air-web-orchestrator`));
    } else {
  ...
  async vendorCrossModuleWorkflows(moduleName) {
    ...
    for (const agentFile of yamlFiles) {
      ...
      for (const item of workflowInstallItems) {
        ...
        // Support both {project-root}/air/... and {project-root}/{air_folder}/... patterns
        const sourceMatch = sourceWorkflowPath.match(/\{project-root\}\/(?:\{air_folder\}|air)\/([^/]+)\/workflows\/(.+)/);
        const installMatch = installWorkflowPath.match(/\{project-root\}\/(?:\{air_folder\}|air)\/([^/]+)\/workflows\/(.+)/);
  ...
  async updateWorkflowConfigSource(workflowYamlPath, newModuleName) {
    ...
    // Support both old format (air) and new format ({air_folder})
    const configSourcePattern = /config_source:\s*["']?\{project-root\}\/(?:\{air_folder\}|air)\/[^/]+\/config\.yaml["']?/g;
    const newConfigSource = `config_source: "{project-root}/{air_folder}/${newModuleName}/config.yaml"`;
  ...
  extractFileReferences(xml) {
    ...
    const patterns = [
      ...
      /\bair\/([^"'\s<>]+)/g, // Direct air/ paths (after {air_folder} replacement)
    ];
    ...
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(xml)) !== null) {
        ...
        // Remove {air_folder} prefix if present (should be rare, mostly replaced already)
        filePath = filePath.replace(/^{air_folder}\//, 'air/');

        // For air/ pattern, prepend 'air/' since it was captured without it
        if (pattern.source.includes(String.raw`\bair\/`)) {
          filePath = 'air/' + filePath;
        }
    ...
    for (const pattern of workflowPatterns) {
      ...
      while ((match = pattern.exec(xml)) !== null) {
        ...
        // Remove {air_folder} prefix if present and replace with air
        workflowPath = workflowPath.replace(/^{air_folder}\//, 'air/');
  ...
  async processFileDependency(filePath, dependencies, processed, moduleName, warnings = []) {
    ...
    if (filePath === 'air/_cfg/agent-manifest.csv' || filePath.endsWith('/agent-manifest.csv')) {
      return;
    }
      ...
      if (depMatch) {
        ...
        if (deps) {
          for (const dep of deps) {
            ...
            depPath = depPath.replace(/^{air_folder}\//, 'air/');
      ...
      if (templateMatch) {
        ...
        if (templates) {
          for (const template of templates) {
            ...
            templatePath = templatePath.replace(/^{air_folder}\//, 'air/');
  ...
  async processWorkflowDependency(workflowPath, dependencies, processed, moduleName, warnings = []) {
    ...
    // Process {project-root} and {air_folder} references in the YAML content
    ...
    if (workflowConfig.web_bundle && workflowConfig.web_bundle.web_bundle_files) {
      ...
      for (const bundleFilePath of bundleFiles) {
        ...
        cleanFilePath = cleanFilePath.replace(/^{air_folder}\//, 'air/');
  ...
  async includeCoreWorkflowFiles(dependencies, processed, moduleName, warnings = []) {
    const coreWorkflowPath = 'air/core/tasks/workflow.xml';
    ...
    // Process {project-root} and {air_folder} references
    fileContent = this.processProjectRootReferences(fileContent);
  ...
  async includeAdvancedElicitationFiles(dependencies, processed, moduleName, warnings = []) {
    const elicitationFiles = ['air/core/tasks/advanced-elicitation.xml', 'air/core/tasks/advanced-elicitation-methods.csv'];
  ...
  async processWildcardDependency(pattern, dependencies, processed, moduleName, warnings = []) {
    ...
    // Replace {air_folder} with air
    pattern = pattern.replace(/^{air_folder}\//, 'air/');
    ...
    let dir;
    if (dirPath.startsWith('air/')) {
      // Remove air/ prefix
      const actualPath = dirPath.replace(/^air\//, '');

      // Try different path mappings for directories
      const possibleDirs = [
        // Try as module path: air/cis/... -> src/modules/cis/...
        path.join(this.sourceDir, 'modules', actualPath),
        // Try as direct path: air/core/... -> src/core/...
  ...
  resolveFilePath(filePath, moduleName) {
    ...
    if (filePath.startsWith('air/_cfg/')) {
    ...
    // Handle different path patterns for air files
    // air/cis/tasks/brain-session.md -> src/modules/cis/tasks/brain-session.md
    // air/core/tasks/create-doc.md -> src/core/tasks/create-doc.md
    // air/aim/templates/brief.md -> src/modules/aim/templates/brief.md

    let actualPath = filePath;

    if (filePath.startsWith('air/')) {
      // Remove air/ prefix
      actualPath = filePath.replace(/^air\//, '');

      // Check if it's a module-specific file (cis, aim, etc) or core file
    ...
    // Try standard paths for non-air files
    const basePaths = [
  ...
  processProjectRootReferences(content) {
    ...
    content = content.replaceAll('{project-root}', '');
    // Replace {air_folder} with air
    content = content.replaceAll('{air_folder}', 'air');
    return content;
  }
