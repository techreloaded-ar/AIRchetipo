...
The AIRchetipo CLI handles installation and web bundling for the AIRchetipo framework. It compiles YAML agents into two distinct formats: **IDE-integrated agents** (filesystem-aware, customizable) and **web bundles** (self-contained, dependency-embedded).

## Table of Contents

- [AIRchetipo CLI Tool](#airchetipo-cli-tool)
...
# Direct CLI usage
node tools/cli/air-cli.js install --target /path/to/project --modules aim,aib --ides codex

# Flags:
#   --target <path>        Target project directory
#   --modules <list>       Comma-separated: aim, aib
...
node tools/cli/bundlers/bundle-web.js module aim       # One module
node tools/cli/bundlers/bundle-web.js agent aim pm     # One agent
```

### Utilities

```bash
npm run air:status              # Installation status
...
3. Install Core + Modules
   - Copy files to {target}/{air_folder}/
...
- Each handler implements: `setup()`, `createArtifacts()`, `cleanup()`, `getAgentsFromAir()`
...
1. **Module-Level**: `src/modules/{module}/_module-installer/platform-specifics/{ide}.js`
   ...
   - Example: AIM creates subagents when installed with Claude Code

2. **IDE-Level**: Embedded in IDE handler's `createArtifacts()` method
   - IDE provides custom handling for specific modules
   - Example: Windsurf configures cascade workflows for AIM
...
The installer generates **5 manifest files** in `{target}/{air_folder}/_cfg/`:
...
- Auto-backup to `.air-backup-{timestamp}` if selected
...
agent:
  ...
  menu:
    - trigger: '*create-brief'
      workflow: '{project-root}/{air_folder}/aim/workflows/.../workflow.yaml'
```

### Output: IDE (Markdown with XML)

````markdown
<!-- Powered by AIRchetipo-CORE -->
...
<agent id="..." name="PM">
  <activation critical="MANDATORY">
    <step n="2">Load {project-root}/{air_folder}/aim/config.yaml at runtime</step>
    ...
  </activation>
  <persona>...</persona>
  <menu>...</menu>
...
<agent id="..." name="PM">
  ...
  <menu>...</menu>
  <bundled-files>
    <file id="air/aim/config.yaml"><![CDATA[...]]></file>
    <file id="air/aim/workflows/.../workflow.yaml"><![CDATA[...]]></file>
    ...
  </bundled-files>
</agent>
````
...
| ----------------------- | ----------------------------------- | --------------------------------- |
| **Trigger**             | `npm run install:air`               | `npm run bundle`                  |
...
| **Output Location**     | `{target}/{air_folder}/` + IDE dirs | `web-bundles/`                    |
...
# Test installation
node tools/cli/air-cli.js install --target ./test-project --modules aim --ides codex

# Test bundling
node tools/cli/bundlers/bundle-web.js agent aim pm
...
- **AIM Workflows**: `src/modules/aim/workflows/README.md`
- **Module Structure**: `src/modules/aib/workflows/create-module/module-structure.md`
- **Agent Creation**: `src/modules/aib/workflows/create-agent/README.md`

---

## Support

- **Issues**: https://github.com/airchetipo-org/AIRchetipo/issues
- **Discord**: https://discord.gg/gk8jAdXWmj (#general-dev, #bugs-issues)
- **YouTube**: https://www.youtube.com/@AIRchetipoCode
