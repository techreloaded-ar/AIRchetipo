...
/**
 * Manages the installation, updating, and removal of AIRchetipo modules.
 ...
 * await manager.install('core-module', '/path/to/air');
 */
class ModuleManager {
  constructor() {
    ...
    this.airFolderName = 'air'; // Default, can be overridden
  }

  /**
   * Set the air folder name for placeholder replacement
   * @param {string} airFolderName - The air folder name
   */
  setAirFolderName(airFolderName) {
    ...
    if (textExtensions.includes(ext)) {
      try {
        ...
        if (content.includes('{air_folder}')) {
          content = content.replaceAll('{air_folder}', this.airFolderName);
        }
   ...
   * @param {string} airDir - Target air directory
   ...
   */
  async install(moduleName, airDir, fileTrackingCallback = null, options = {}) {
    const sourcePath = path.join(this.modulesSourcePath, moduleName);
    const targetPath = path.join(airDir, moduleName);
    ...
    if (!options.skipModuleInstaller) {
      await this.runModuleInstaller(moduleName, airDir, options);
    }
   ...
   */
  async update(moduleName, airDir, force = false) {
    const sourcePath = path.join(this.modulesSourcePath, moduleName);
    const targetPath = path.join(airDir, moduleName);
    ...
    if (force) {
      ...
      await fs.remove(targetPath);
      return await this.install(moduleName, airDir);
    } else {
   ...
   * @param {string} airDir - Target air directory
   */
  async remove(moduleName, airDir) {
    const targetPath = path.join(airDir, moduleName);
   ...
   */
  async isInstalled(moduleName, airDir) {
    const targetPath = path.join(airDir, moduleName);
   ...
   */
  async getInstalledInfo(moduleName, airDir) {
    const targetPath = path.join(airDir, moduleName);
  ...
  async copyWorkflowYamlStripped(sourceFile, targetFile) {
    ...
    // IMPORTANT: Replace {air_folder} BEFORE parsing YAML
    // Otherwise parsing will fail on the placeholder
    yamlContent = yamlContent.replaceAll('{air_folder}', this.airFolderName);
  ...
  async vendorCrossModuleWorkflows(sourcePath, targetPath, moduleName) {
    ...
    for (const agentFile of yamlFiles) {
      ...
      for (const item of workflowInstallItems) {
        ...
        // Handle both {air_folder} placeholder and hardcoded 'air'
        // Example: {project-root}/{air_folder}/aim/workflows/4-implementation/create-story/workflow.yaml
        // Or: {project-root}/air/aim/workflows/4-implementation/create-story/workflow.yaml
        ...
        // Handle both {air_folder} placeholder and hardcoded 'air'
        // Example: {project-root}/{air_folder}/aim/workflows/architecture/workflow.yaml
  ...
  async updateWorkflowConfigSource(workflowYamlPath, newModuleName) {
    ...
    // Replace config_source: "{project-root}/{air_folder}/OLD_MODULE/config.yaml"
    // with config_source: "{project-root}/{air_folder}/NEW_MODULE/config.yaml"
    // Note: At this point {air_folder} has already been replaced with actual folder name
    const configSourcePattern = /config_source:\s*["']?\{project-root\}\/[^/]+\/[^/]+\/config\.yaml["']?/g;
    const newConfigSource = `config_source: "{project-root}/${this.airFolderName}/${newModuleName}/config.yaml"`;

    const updatedYaml = yamlContent.replaceAll(configSourcePattern, newConfigSource);

    if (updatedYaml !== yamlContent) {
      await fs.writeFile(workflowYamlPath, updatedYaml, 'utf8');
      console.log(chalk.dim(`      Updated config_source to: ${this.airFolderName}/${newModuleName}/config.yaml`));
    }
  }
   ...
   */
  async runModuleInstaller(moduleName, airDir, options = {}) {
    ...
    try {
      ...
      if (typeof moduleInstaller.install === 'function') {
        // Get project root (parent of air directory)
        const projectRoot = path.dirname(airDir);
  ...
  async processModuleConfig(modulePath, moduleName) {
    ...
    if (await fs.pathExists(configPath)) {
      try {
        ...
        configContent = configContent.replaceAll('{project-root}', `air/${moduleName}`);
