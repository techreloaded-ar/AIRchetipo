...
class ManifestGenerator {
   ...
   */
  async generateManifests(airDir, selectedModules, installedFiles = [], options = {}) {
    ...
    this.airDir = airDir;
    this.airFolderName = path.basename(airDir); // Get the actual folder name (e.g., '.air' or 'air')
   ...
   * Scans the INSTALLED air directory, not the source
   */
  async collectWorkflows(selectedModules) {
    ...
    for (const moduleName of this.updatedModules) {
      const modulePath = path.join(this.airDir, moduleName);
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (workflow.name && workflow.description) {
              ...
              const installPath =
                moduleName === 'core'
                  ? `${this.airFolderName}/core/workflows/${relativePath}/workflow.yaml`
                  : `${this.airFolderName}/${moduleName}/workflows/${relativePath}/workflow.yaml`;
  ...
  async collectAgents(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const agentsPath = path.join(this.airDir, moduleName, 'agents');
    ...
    const standaloneAgentsDir = path.join(this.airDir, 'agents');
  ...
  async getAgentsFromDir(dirPath, moduleName) {
    ...
    for (const file of files) {
      ...
      if (file.endsWith('.md') && !file.endsWith('.agent.yaml') && file.toLowerCase() !== 'readme.md') {
        ...
        // Build relative path for installation
        const installPath =
          moduleName === 'core' ? `${this.airFolderName}/core/agents/${file}` : `${this.airFolderName}/${moduleName}/agents/${file}`;
  ...
  async collectTasks(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const tasksPath = path.join(this.airDir, moduleName, 'tasks');
  ...
  async getTasksFromDir(dirPath, moduleName) {
    ...
    for (const file of files) {
      ...
      if (file.endsWith('.xml') || file.endsWith('.md')) {
        ...
        // Build relative path for installation
        const installPath =
          moduleName === 'core' ? `${this.airFolderName}/core/tasks/${file}` : `${this.airFolderName}/${moduleName}/tasks/${file}`;
  ...
  async collectTools(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const toolsPath = path.join(this.airDir, moduleName, 'tools');
  ...
  async getToolsFromDir(dirPath, moduleName) {
    ...
    for (const file of files) {
      ...
      if (file.endsWith('.xml') || file.endsWith('.md')) {
        ...
        // Build relative path for installation
        const installPath =
          moduleName === 'core' ? `${this.airFolderName}/core/tools/${file}` : `${this.airFolderName}/${moduleName}/tools/${file}`;
  ...
  async writeFilesManifest(cfgDir) {
    ...
    if (this.allInstalledFiles && this.allInstalledFiles.length > 0) {
      ...
      for (const filePath of this.allInstalledFiles) {
        const relativePath = 'air' + filePath.replace(this.airDir, '').replaceAll('\\', '/');
      ...
      for (const file of this.files) {
        const filePath = path.join(this.airDir, file.path.replace(this.airFolderName + '/', ''));
        const hash = await this.calculateFileHash(filePath);
        allFiles.push({
          ...file,
          hash: hash,
