...
class Installer {
  ...
  /**
   * Find the air installation directory in a project
   * V6+ installations can use ANY folder name but ALWAYS have _cfg/manifest.yaml
   * @param {string} projectDir - Project directory
   * @returns {Promise<string>} Path to air directory
   */
  async findAIRchetipoDir(projectDir) {
    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      // Project doesn't exist yet, return default
      return path.join(projectDir, 'air');
    }
  ...
  /**
   * Copy a file and replace {air_folder} placeholder with actual folder name
   * @param {string} sourcePath - Source file path
   * @param {string} targetPath - Target file path
   * @param {string} airFolderName - The air folder name to use for replacement
   */
  async copyFileWithPlaceholderReplacement(sourcePath, targetPath, airFolderName) {
    ...
    if (textExtensions.includes(ext)) {
      try {
        ...
        // Replace {air_folder} placeholder with actual folder name
        if (content.includes('{air_folder}')) {
          content = content.replaceAll('{air_folder}', airFolderName);
        }
  ...
  async collectToolConfigurations(projectDir, selectedModules, isFullReinstall = false, previousIdes = [], preSelectedIdes = null) {
    ...
    const airDir = path.join(projectDir, this.airFolderName || 'air');

    // During full reinstall, use the saved previous IDEs since air dir was deleted
    ...
    if (isFullReinstall) {
      ...
      const existingInstall = await detector.detect(airDir);
      previouslyConfiguredIdes = existingInstall.ides || [];
    }

    // Load saved IDE configurations for already-configured IDEs
    const savedIdeConfigs = await this.ideConfigManager.loadAllIdeConfigs(airDir);
  ...
  async install(config) {
    ...
    CLIUtils.displaySection('AIRCHETIPO™ Installation', 'Version ' + require(path.join(getProjectRoot(), 'package.json')).version);
    ...
    // Get air_folder from config (default to 'air' for backwards compatibility)
    const airFolderName = moduleConfigs.core && moduleConfigs.core.air_folder ? moduleConfigs.core.air_folder : 'air';
    this.airFolderName = airFolderName; // Store for use in other methods

    // Set air folder name on module manager and IDE manager for placeholder replacement
    this.moduleManager.setAirFolderName(airFolderName);
    this.ideManager.setAirFolderName(airFolderName);
    ...
    try {
      ...
      // Check if air_folder has changed from existing installation (only if project dir exists)
      let existingAIRchetipoDir = null;
      let existingAIRchetipoFolderName = null;

      if (await fs.pathExists(projectDir)) {
        existingAIRchetipoDir = await this.findAIRchetipoDir(projectDir);
        existingAIRchetipoFolderName = path.basename(existingAIRchetipoDir);
      }

      const targetAIRchetipoDir = path.join(projectDir, airFolderName);

      // If air_folder changed during update/upgrade, back up old folder and do fresh install
      if (existingAIRchetipoDir && (await fs.pathExists(existingAIRchetipoDir)) && existingAIRchetipoFolderName !== airFolderName) {
        spinner.stop();
        console.log(chalk.yellow(`\n⚠️  air_folder has changed: ${existingAIRchetipoFolderName} → ${airFolderName}`));
        ...
        let backupDir = `${existingAIRchetipoDir}-bak`;
        let counter = 1;
        while (await fs.pathExists(backupDir)) {
          backupDir = `${existingAIRchetipoDir}-bak-${counter}`;
          counter++;
        }

        // Rename the old folder to backup
        await fs.move(existingAIRchetipoDir, backupDir);

        spinner.succeed(`Backed up ${existingAIRchetipoFolderName} → ${path.basename(backupDir)}`);
        console.log(chalk.cyan('\n📋 Important:'));
        ...
        console.log(chalk.dim(`    ${airFolderName}/_cfg/`));
        console.log('');
      ...
      const airDir = path.join(projectDir, airFolderName);

      // Check existing installation
      spinner.text = 'Checking for existing installation...';
      const existingInstall = await this.detector.detect(airDir);
      ...
      if (existingInstall.installed && !config.force && !config._quickUpdate) {
        ...
        if (config._requestedReinstall) {
          ...
          console.log(chalk.yellow('\n⚠️  Existing AIRCHETIPO installation detected'));
          console.log(chalk.dim(`  Location: ${airDir}`));
        ...
        if (action === 'reinstall') {
          ...
          console.log(chalk.red('All custom files and modifications in the air directory will be lost.'));
          ...
          const existingFilesManifest = await this.readFilesManifest(airDir);
          ...
          const { customFiles, modifiedFiles } = await this.detectCustomFiles(airDir, existingFilesManifest);
          ...
          if (customFiles.length > 0) {
            const tempBackupDir = path.join(projectDir, '.air-custom-backup-temp');
            await fs.ensureDir(tempBackupDir);

            spinner.start(`Backing up ${customFiles.length} custom files...`);
            for (const customFile of customFiles) {
              const relativePath = path.relative(airDir, customFile);
          ...
          if (modifiedFiles.length > 0) {
            const tempModifiedBackupDir = path.join(projectDir, '.air-modified-backup-temp');
            await fs.ensureDir(tempModifiedBackupDir);

            console.log(chalk.yellow(`\nDEBUG: Backing up ${modifiedFiles.length} modified files to temp location`));
            spinner.start(`Backing up ${modifiedFiles.length} modified files...`);
            for (const modifiedFile of modifiedFiles) {
              const relativePath = path.relative(airDir, modifiedFile.path);
      ...
      await this.createDirectoryStructure(airDir);
      ...
      if (config.installCore || resolution.byModule.core) {
        spinner.start('Installing AIRCHETIPO core...');
        await this.installCoreWithDependencies(airDir, resolution.byModule.core);
      ...
      if (config.modules && config.modules.length > 0) {
        for (const moduleName of config.modules) {
          ...
          await this.installModuleWithDependencies(moduleName, airDir, resolution.byModule[moduleName]);
        ...
        for (const [module, files] of Object.entries(resolution.byModule)) {
          if (!config.modules.includes(module) && module !== 'core') {
            ...
            if (totalFiles > 0) {
              ...
              await this.installPartialModule(module, airDir, files);
      ...
      await this.generateModuleConfigs(airDir, moduleConfigs);
      ...
      const cfgDir = path.join(airDir, '_cfg');
      ...
      const manifestStats = await manifestGen.generateManifests(airDir, config.modules || [], this.installedFiles, {
        ides: config.ides || [],
        preservedModules: config._preserveModules || [], // Scan these from installed air/ dir
      });
      ...
      if (!config.skipIde && config.ides && config.ides.length > 0) {
        ...
        if (validIdes.length === 0) {
          ...
          for (const ide of validIdes) {
            ...
            await this.ideManager.setup(ide, projectDir, airDir, {
            ...
            if (ideConfigurations[ide] && !ideConfigurations[ide]._alreadyConfigured) {
              await this.ideConfigManager.saveIdeConfig(airDir, ide, ideConfigurations[ide]);
            }
        ...
        if (validIdesForDocs.length > 0) {
          ...
          await this.copyIdeDocumentation(validIdesForDocs, airDir);
      ...
      if (config.installCore || resolution.byModule.core) {
        ...
        await this.moduleManager.runModuleInstaller('core', airDir, {
      ...
      if (config.modules && config.modules.length > 0) {
        for (const moduleName of config.modules) {
          ...
          await this.moduleManager.runModuleInstaller(moduleName, airDir, {
            installedIDEs: config.ides || [],
      ...
      if (config._isUpdate) {
        if (config._customFiles && config._customFiles.length > 0) {
          ...
          for (const originalPath of config._customFiles) {
            const relativePath = path.relative(airDir, originalPath);
      ...
      if (customFiles.length > 0) {
        ...
        for (const file of customFiles) {
          console.log(chalk.dim(`  - ${path.relative(airDir, file)}`));
        }
        console.log('');
      }
      ...
      return { success: true, path: airDir, modules: config.modules, ides: config.ides };
    } catch (error) {
  ...
  async update(config) {
    ...
    try {
      const projectDir = path.resolve(config.directory);
      const airDir = await this.findAIRchetipoDir(projectDir);
      const existingInstall = await this.detector.detect(airDir);

      if (!existingInstall.installed) {
        spinner.fail('No AIRCHETIPO installation found');
        throw new Error(`No AIRCHETIPO installation found at ${airDir}`);
      }
      ...
      if (existingInstall.hasCore) {
        ...
        await this.updateCore(airDir, config.force);
      }

      for (const module of existingInstall.modules) {
        spinner.text = `Updating module: ${module.id}...`;
        await this.moduleManager.update(module.id, airDir, config.force);
      }

      // Update manifest
      spinner.text = 'Updating manifest...';
      await this.manifest.update(airDir, {
        version: newVersion,
  ...
  async getStatus(directory) {
    ...
    const airDir = await this.findAIRchetipoDir(projectDir);
    return await this.detector.detect(airDir);
  }
   ...
   */
  async createDirectoryStructure(airDir) {
    await fs.ensureDir(airDir);
    await fs.ensureDir(path.join(airDir, '_cfg'));
    await fs.ensureDir(path.join(airDir, '_cfg', 'agents'));
  }
   ...
   */
  async generateModuleConfigs(airDir, moduleConfigs) {
    ...
    const entries = await fs.readdir(airDir, { withFileTypes: true });
    ...
    for (const moduleName of installedModules) {
      const modulePath = path.join(airDir, moduleName);
...
# Generated by AIRCHETIPO installer
...
`;
   ...
   */
  async installCoreWithDependencies(airDir, coreFiles) {
    const sourcePath = getModulePath('core');
    const targetPath = path.join(airDir, 'core');

    // Install full core
    await this.installCore(airDir);
   ...
   */
  async installModuleWithDependencies(moduleName, airDir, moduleFiles) {
   ...
   */
  async installPartialModule(moduleName, airDir, files) {
    const sourceBase = getModulePath(moduleName);
    const targetBase = path.join(airDir, moduleName);
    ...
    if (files.agents && files.agents.length > 0) {
      ...
      for (const agentPath of files.agents) {
        ...
        if (await fs.pathExists(sourcePath)) {
          await this.copyFileWithPlaceholderReplacement(sourcePath, targetPath, this.airFolderName || 'air');
    ...
    if (files.data && files.data.length > 0) {
      for (const dataPath of files.data) {
        ...
        if (await fs.pathExists(dataPath)) {
          await this.copyFileWithPlaceholderReplacement(dataPath, targetPath, this.airFolderName || 'air');
          this.installedFiles.push(targetPath);
        }
      }
    }
  ...
  async copyDirectoryWithFiltering(sourcePath, targetPath) {
    ...
    for (const file of files) {
      ...
      await this.copyFileWithPlaceholderReplacement(sourceFile, targetFile, this.airFolderName || 'air');
   ...
   * @param {string} modulePath - Path to module in air/ installation
   * @param {string} moduleName - Module name
   */
  async processAgentFiles(modulePath, moduleName) {
    ...
    // Determine project directory (parent of air/ directory)
    const airDir = path.dirname(modulePath);
    const projectDir = path.dirname(airDir);
    const cfgAgentsDir = path.join(airDir, '_cfg', 'agents');
      ...
      if (agentFile.endsWith('.agent.yaml')) {
        ...
        if (!(await fs.pathExists(customizePath))) {
          ...
          if (await fs.pathExists(genericTemplatePath)) {
            await this.copyFileWithPlaceholderReplacement(genericTemplatePath, customizePath, this.airFolderName || 'air');
        ...
        // Write the built .md file to air/{module}/agents/ with POSIX-compliant final newline
  ...
  /**
   * Build standalone agents in air/agents/ directory
   * @param {string} airDir - Path to air directory
   * @param {string} projectDir - Path to project directory
   */
  async buildStandaloneAgents(airDir, projectDir) {
    const standaloneAgentsPath = path.join(airDir, 'agents');
    const cfgAgentsDir = path.join(airDir, '_cfg', 'agents');
  ...
  async compileAgents(config) {
    ...
    try {
      ...
      if (!(await fs.pathExists(airDir))) {
        spinner.fail('No AIRCHETIPO installation found');
        throw new Error(`AIRCHETIPO not installed at ${airDir}`);
      }

      let agentCount = 0;
      let taskCount = 0;

      // Process all modules in air directory
      spinner.text = 'Rebuilding agent files...';
      const entries = await fs.readdir(airDir, { withFileTypes: true });

      for (const entry of entries) {
        if (entry.isDirectory() && entry.name !== '_cfg' && entry.name !== 'docs') {
          const modulePath = path.join(airDir, entry.name);

          // Special handling for standalone agents in air/agents/ directory
          if (entry.name === 'agents') {
            spinner.text = 'Building standalone agents...';
            await this.buildStandaloneAgents(airDir, projectDir);
      ...
      const existingManifestPath = path.join(airDir, '_cfg', 'manifest.yaml');
      let existingIdes = [];
      ...
      await manifestGen.generateManifests(airDir, installedModules, [], {
        ides: existingIdes,
      });
   ...
   * Private: Update core
   */
  async updateCore(airDir, force = false) {
    const sourcePath = getModulePath('core');
    const targetPath = path.join(airDir, 'core');
  ...
  async quickUpdate(config) {
    ...
    try {
      ...
      if (!(await fs.pathExists(airDir))) {
        spinner.fail('No AIRCHETIPO installation found');
        throw new Error(`AIRCHETIPO not installed at ${airDir}. Use regular install for first-time setup.`);
      }
      ...
      spinner.start('Updating AIRCHETIPO installation...');
  ...
  /**
   * Handle legacy AIRCHETIPO v4 migration with automatic backup
   * @param {string} projectDir - Project directory
   * @param {Object} legacyV4 - Legacy V4 detection result with offenders array
   */
  async handleLegacyV4Migration(projectDir, legacyV4) {
    console.log(chalk.yellow.bold('\n⚠️  Legacy AIRCHETIPO v4 detected'));
    console.log(chalk.dim('The installer found legacy artefacts in your project.\n'));

    // Separate .air* folders (auto-backup) from other offending paths (manual cleanup)
    const airFolders = legacyV4.offenders.filter((p) => {
      const name = path.basename(p);
      return name.startsWith('.air'); // Only dot-prefixed folders get auto-backed up
    });
    const otherOffenders = legacyV4.offenders.filter((p) => {
      const name = path.basename(p);
      return !name.startsWith('.air'); // Everything else is manual cleanup
    });
    ...
    // Handle .air* folders with automatic backup
    if (airFolders.length > 0) {
      console.log(chalk.cyan('The following legacy folders will be moved to v4-backup:'));
      for (const p of airFolders) console.log(chalk.dim(` - ${p}`));
      ...
      if (proceed) {
        ...
        for (const folder of airFolders) {
   ...
   */
  async readFilesManifest(airDir) {
    const filesManifestPath = path.join(airDir, '_cfg', 'files-manifest.csv');
   ...
   */
  async detectCustomFiles(airDir, existingFilesManifest) {
    const customFiles = [];
    const modifiedFiles = [];
    ...
    for (const fileEntry of existingFilesManifest) {
      if (fileEntry.path) {
        // Files in manifest are stored as relative paths starting with 'air/'
        // Convert to absolute path
        const relativePath = fileEntry.path.startsWith('air/') ? fileEntry.path.slice(5) : fileEntry.path;
        const absolutePath = path.join(airDir, relativePath);
    ...
    // Recursively scan airDir for all files
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            const relativePath = path.relative(airDir, fullPath);
   ...
   */
  async createAgentConfigs(airDir, userInfo = null) {
    const agentConfigDir = path.join(airDir, '_cfg', 'agents');
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== '_cfg') {
        const moduleAgentsPath = path.join(airDir, entry.name, 'agents');
    ...
    await this.generateAgentManifest(airDir, agentDetails);
   ...
   */
  async generateAgentManifest(airDir, agentDetails) {
    const manifestPath = path.join(airDir, '_cfg', 'agent-manifest.csv');
  ...
  /**
   * Copy IDE-specific documentation to AIRCHETIPO docs
   * @param {Array} ides - List of selected IDEs
   * @param {string} airDir - AIRCHETIPO installation directory
   */
  async copyIdeDocumentation(ides, airDir) {
    const docsDir = path.join(airDir, 'docs');
    ...
    for (const ide of ides) {
      ...
      if (await fs.pathExists(sourceDocPath)) {
        await this.copyFileWithPlaceholderReplacement(sourceDocPath, targetDocPath, this.airFolderName || 'air');
      }
    }
  }
}
