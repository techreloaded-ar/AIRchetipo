...
class ConfigCollector {
  ...
  async loadExistingConfig(projectDir) {
    ...
    // Find the actual air directory (handles custom folder names)
    ...
    // Dynamically discover all installed modules by scanning air directory
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        const moduleConfigPath = path.join(airDir, entry.name, 'config.yaml');
    ...
    if (foundAny) {
      console.log(chalk.cyan('\n📋 Found existing AIRCHETIPO module configurations'));
    }
