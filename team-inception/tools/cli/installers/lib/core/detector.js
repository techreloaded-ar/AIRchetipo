...
class Detector {
  /**
   * Detect existing AIRCHETIPO installation
  ...
  async detect(airDir) {
    ...
    const manifestData = await manifest.read(airDir);
    if (manifestData) {
    ...
    // Check for core
    const corePath = path.join(airDir, 'core');
    ...
    if (manifestData && manifestData.modules && manifestData.modules.length > 0) {
      ...
      for (const moduleId of manifestData.modules) {
        const modulePath = path.join(airDir, moduleId);
  ...
  /**
   * Detect legacy installation (.air-method, .aim, .cis)
  ...
  async detectLegacy(projectDir) {
    ...
    // Check for legacy core (.air-method)
    const legacyCorePath = path.join(projectDir, '.air-method');
    ...
    for (const entry of entries) {
      if (
        ...
        // Check if it's likely a AIRCHETIPO module
  ...
  async checkMigrationNeeded(projectDir) {
    const airDir = path.join(projectDir, 'air');
    const current = await this.detect(airDir);
  ...
  /**
   * Detect legacy AIRCHETIPO v4 footprints (case-sensitive path checks)
   * V4 used .air-method as default folder name
  ...
  async detectLegacyV4(projectDir) {
    ...
    // 3. If NO V6+ found → flag folders with "air" in name as potential V4 legacy
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          // This works for ANY folder name (not just air-prefixed)
          ...
          if (isV6) {
            ...
            // Not V6+, check if folder name contains "air" (case insensitive)
            const nameLower = name.toLowerCase();
            if (nameLower.includes('air')) {
    ...
    // Check inside various IDE command folders for legacy air folders
    // V4 used folders like 'airchetipo' or custom names in IDE commands
    // V6+ uses 'air' in IDE commands (hardcoded in IDE handlers)
      ...
      if (await fs.pathExists(commandsPath)) {
        try {
          ...
          for (const entry of commandEntries) {
            if (entry.isDirectory()) {
              const name = entry.name;
              // V4 used 'airchetipo' or similar in IDE commands folders
              // V6+ uses 'air' (hardcoded)
              // So anything that's NOT 'air' but starts with air/AIRchetipo is likely V4
              if ((name.startsWith('air') || name.startsWith('AIRchetipo') || name === 'AIRchetipo') && name !== 'air') {
