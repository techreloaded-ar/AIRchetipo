...
/**
 * Dependency Resolver for AIRCHETIPO modules
...
class DependencyResolver {
   ...
   */
  async resolve(airDir, selectedModules = [], options = {}) {
    if (options.verbose) {
    ...
    const primaryFiles = await this.collectPrimaryFiles(airDir, modulesToProcess);
    ...
    const resolvedDeps = await this.resolveDependencyPaths(airDir, allDependencies);

    // Fourth pass: check for transitive dependencies
    const transitiveDeps = await this.resolveTransitiveDependencies(airDir, resolvedDeps);

    // Combine all files
    const allFiles = new Set([...primaryFiles.map((f) => f.path), ...resolvedDeps, ...transitiveDeps]);

    // Organize by module
    const organizedFiles = this.organizeByModule(airDir, allFiles);
   ...
   */
  async collectPrimaryFiles(airDir, modules) {
    const files = [];

    for (const module of modules) {
      // Handle both source (src/) and installed (air/) directory structures
      let moduleDir;

      // Check if this is a source directory (has 'src' subdirectory)
      const srcDir = path.join(airDir, 'src');
      if (await fs.pathExists(srcDir)) {
        ...
        // Installed directory structure: air/core or air/modules/xxx
        moduleDir = module === 'core' ? path.join(airDir, 'core') : path.join(airDir, 'modules', module);
      }
  ...
  async parseDependencies(files) {
    ...
    for (const file of files) {
      ...
      for (const ref of fileRefs) {
        ...
        // Paths starting with air/ are absolute references to the air installation
        const depType = ref.startsWith('air/') ? 'air-path' : 'file';
        allDeps.add({
          from: file.path,
          dependency: ref,
          type: depType,
  ...
  parseCommandReferences(content) {
    ...
    const commandPattern = /@(task-|agent-|air-)([a-z0-9-]+)/g;
    let match;
    ...
    // Match file paths like air/core/agents/analyst
    const pathPattern = /air\/(core|aim|cis)\/(agents|tasks)\/([a-z0-9-]+)/g;
  ...
  parseFileReferences(content) {
    ...
    while ((match = execPattern.exec(content)) !== null) {
      ...
      if (execPath && execPath !== '*') {
        ...
        // Usage is like {project-root}/air/core/tasks/foo.md
   ...
   */
  async resolveDependencyPaths(airDir, dependencies) {
    const resolved = new Set();

    for (const dep of dependencies) {
      const resolvedPaths = await this.resolveSingleDependency(airDir, dep);
   ...
   */
  async resolveSingleDependency(airDir, dep) {
    const paths = [];
    ...
    switch (dep.type) {
      ...
      case 'file': {
        ...
        if (depPath.includes('{project-root}')) {
          // Remove {project-root} and resolve as air path
          depPath = depPath.replace('{project-root}', '');

          if (depPath.startsWith('air/')) {
            const airPath = depPath.replace(/^air\//, '');

            // Handle glob patterns
            if (depPath.includes('*')) {
              // Extract the base path and pattern
              const pathParts = airPath.split('/');
              ...
              let basePath;
              if (module === 'core') {
                basePath = path.join(airDir, 'core', middlePath);
              } else {
                basePath = path.join(airDir, 'modules', module, middlePath);
              }
              ...
              // Direct path
              if (airPath.startsWith('core/')) {
                const corePath = path.join(airDir, airPath);
                if (await fs.pathExists(corePath)) {
                  paths.push(corePath);
                }
              } else {
                const parts = airPath.split('/');
                const module = parts[0];
                const rest = parts.slice(1).join('/');
                const modulePath = path.join(airDir, 'modules', module, rest);
      ...
      case 'command': {
        ...
        const commandPath = await this.resolveCommandToPath(airDir, dep.dependency);
        if (commandPath) {
      ...
      case 'air-path': {
        // Resolve air/ paths (from {project-root}/air/... references)
        // These are paths relative to the src directory structure
        const airPath = dep.dependency.replace(/^air\//, '');

        // Try to resolve as if it's in src structure
        // air/core/tasks/foo.md -> src/core/tasks/foo.md
        // air/aim/tasks/bar.md -> src/modules/aim/tasks/bar.md
        ...
        if (airPath.startsWith('core/')) {
          ...
          // It's a module path like aim/tasks/foo.md or cis/agents/bar.md
          const parts = airPath.split('/');
          const module = parts[0];
          const rest = parts.slice(1).join('/');
          const modulePath = path.join(airDir, 'modules', module, rest);
      ...
      case 'template': {
        ...
        if (templateDep.includes('{project-root}')) {
          // Remove {project-root} and treat as air-path
          templateDep = templateDep.replace('{project-root}', '');

          // Now resolve as a air path
          if (templateDep.startsWith('air/')) {
            const airPath = templateDep.replace(/^air\//, '');
   ...
   */
  async resolveCommandToPath(airDir, command) {
    // Parse command format: @task-name or @agent-name or air/module/type/name

    if (command.startsWith('@task-')) {
      const taskName = command.slice(6);
      // Search all modules for this task
      for (const module of ['core', 'aim', 'cis']) {
        const taskPath =
          module === 'core'
            ? path.join(airDir, 'core', 'tasks', `${taskName}.md`)
            : path.join(airDir, 'modules', module, 'tasks', `${taskName}.md`);
      ...
      for (const module of ['core', 'aim', 'cis']) {
        const agentPath =
          module === 'core'
            ? path.join(airDir, 'core', 'agents', `${agentName}.md`)
            : path.join(airDir, 'modules', module, 'agents', `${agentName}.md`);
      ...
      if (parts.length >= 4) {
        ...
        const filePath =
          module === 'core' ? path.join(airDir, 'core', type, fileName) : path.join(airDir, 'modules', module, type, fileName);
   ...
   */
  async resolveTransitiveDependencies(airDir, directDeps) {
        ...
        const subDeps = await this.parseDependencies([
          {
            path: depPath,
            type: 'dependency',
            module: this.getModuleFromPath(airDir, depPath),
            name: path.basename(depPath),
          },
        ]);

        const resolvedSubDeps = await this.resolveDependencyPaths(airDir, subDeps);
   ...
   */
  getModuleFromPath(airDir, filePath) {
    const relative = path.relative(airDir, filePath);
   ...
   */
  organizeByModule(airDir, files) {
    const organized = {};

    for (const file of files) {
      const module = this.getModuleFromPath(airDir, file);
      if (!organized[module]) {
        organized[module] = {
          agents: [],
          tasks: [],
      ...
      if (file.includes('/src/core/') || file.includes('/src/modules/')) {
        moduleBase = module === 'core' ? path.join(airDir, 'src', 'core') : path.join(airDir, 'src', 'modules', module);
      } else {
        // Installed structure
        moduleBase = module === 'core' ? path.join(airDir, 'core') : path.join(airDir, 'modules', module);
      }
