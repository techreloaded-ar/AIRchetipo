...
class Manifest {
   ...
   */
  async create(airDir, data, installedFiles = []) {
    const manifestPath = path.join(airDir, '_cfg', 'manifest.yaml');
   ...
   */
  async read(airDir) {
    const yamlPath = path.join(airDir, '_cfg', 'manifest.yaml');
    const yaml = require('js-yaml');
   ...
   */
  async update(airDir, updates, installedFiles = null) {
    const yaml = require('js-yaml');
    const manifest = (await this.read(airDir)) || {};
   ...
   */
  async addModule(airDir, moduleName) {
    const manifest = await this.read(airDir);
    if (!manifest) {
    ...
    if (!manifest.modules.includes(moduleName)) {
      ...
      await this.update(airDir, { modules: manifest.modules });
    }
  }
   ...
   */
  async removeModule(airDir, moduleName) {
    const manifest = await this.read(airDir);
  ...
  async addIde(airDir, ideName) {
    ...
    if (!manifest.ides.includes(ideName)) {
      ...
      await this.update(airDir, { ides: manifest.ides });
    }
  }
   ...
   * @param {string} airDir - Path to air directory for relative paths
   * @returns {Array} Array of file metadata objects
   */
  async parseInstalledFiles(installedFiles, airDir) {
    const fileMetadata = [];

    for (const filePath of installedFiles) {
      const fileExt = path.extname(filePath).toLowerCase();
      // Make path relative to parent of air directory, starting with 'air/'
      const relativePath = 'air' + filePath.replace(airDir, '').replaceAll('\\', '/');
   ...
   * @param {string} relativePath - Relative path starting with 'air/'
