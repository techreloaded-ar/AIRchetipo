...
class GeminiSetup extends BaseIdeSetup {
  ...
  /**
   * Load config values from air installation
   * @param {string} airDir - AIRCHETIPO installation directory
   * @returns {Object} Config values
   */
  async loadConfigValues(airDir) {
    const configValues = {
      user_name: 'User', // Default fallback
    };

    // Try to load core config.yaml
    const coreConfigPath = path.join(airDir, 'core', 'config.yaml');
  ...
  async setup(projectDir, airDir, options = {}) {
    ...
    // Create .gemini/commands directory (flat structure with air- prefix)
    ...
    // Install agents as TOML files with air- prefix (flat structure)
    let agentCount = 0;
    for (const artifact of agentArtifacts) {
      const tomlContent = await this.createAgentLauncherToml(artifact);

      // Flat structure: air-agent-{module}-{name}.toml
      const tomlPath = path.join(commandsDir, `air-agent-${artifact.module}-${artifact.name}.toml`);
      await this.writeFile(tomlPath, tomlContent);
      agentCount++;

      console.log(chalk.green(`  ✓ Added agent: /air:agents:${artifact.module}:${artifact.name}`));
    }

    // Install tasks as TOML files with air- prefix (flat structure)
    let taskCount = 0;
    for (const task of tasks) {
      const content = await this.readFile(task.path);
      const tomlContent = await this.createTaskToml(task, content);

      // Flat structure: air-task-{module}-{name}.toml
      const tomlPath = path.join(commandsDir, `air-task-${task.module}-${task.name}.toml`);
      await this.writeFile(tomlPath, tomlContent);
      taskCount++;

      console.log(chalk.green(`  ✓ Added task: /air:tasks:${task.module}:${task.name}`));
    }
    ...
    console.log(chalk.dim(`  - Agent activation: /air:agents:{agent-name}`));
    console.log(chalk.dim(`  - Task activation: /air:tasks:{task-name}`));
  ...
  async createAgentLauncherToml(artifact) {
    ...
    const description = `AIRCHETIPO ${artifact.module.toUpperCase()} Agent: ${title}`;
...
`;
  ...
  /**
   * Cleanup Gemini configuration - surgically remove only AIRCHETIPO files
   */
  async cleanup(projectDir) {
    ...
    if (await fs.pathExists(commandsDir)) {
      ...
      for (const file of files) {
        if (file.startsWith('air-') && file.endsWith('.toml')) {
