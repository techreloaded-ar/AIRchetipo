...
class CursorSetup extends BaseIdeSetup {
  ...
  /**
   * Cleanup old AIRCHETIPO installation before reinstalling
   * @param {string} projectDir - Project directory
   */
  async cleanup(projectDir) {
    const fs = require('fs-extra');
    const airRulesDir = path.join(projectDir, this.configDir, this.rulesDir, 'air');

    if (await fs.pathExists(airRulesDir)) {
      await fs.remove(airRulesDir);
      console.log(chalk.dim(`  Removed old AIRCHETIPO rules from ${this.name}`));
    }
  }
  ...
  async setup(projectDir, airDir, options = {}) {
    ...
    const airRulesDir = path.join(rulesDir, 'air');

    await this.ensureDir(airRulesDir);
    ...
    for (const module of modules) {
      await this.ensureDir(path.join(airRulesDir, module));
      await this.ensureDir(path.join(airRulesDir, module, 'agents'));
      await this.ensureDir(path.join(airRulesDir, module, 'tasks'));
      await this.ensureDir(path.join(airRulesDir, module, 'tools'));
      await this.ensureDir(path.join(airRulesDir, module, 'workflows'));
    }
    ...
    for (const artifact of agentArtifacts) {
      ...
      const targetPath = path.join(airRulesDir, artifact.module, 'agents', `${artifact.name}.mdc`);
    ...
    for (const task of tasks) {
      ...
      const targetPath = path.join(airRulesDir, task.module, 'tasks', `${task.name}.mdc`);
    ...
    for (const tool of tools) {
      ...
      const targetPath = path.join(airRulesDir, tool.module, 'tools', `${tool.name}.mdc`);
    ...
    for (const workflow of workflows) {
      ...
      const targetPath = path.join(airRulesDir, workflow.module, 'workflows', `${workflow.name}.mdc`);

      await this.writeFile(targetPath, content);
      workflowCount++;
    }

    // Create AIRCHETIPO index file (but NOT .cursorrules - user manages that)
    await this.createAIRCHETIPOIndex(airRulesDir, agents, tasks, tools, workflows, modules);
    ...
    console.log(chalk.dim(`  - Rules directory: ${path.relative(projectDir, airRulesDir)}`));
  ...
  /**
   * Create AIRCHETIPO index file for easy navigation
   */
  async createAIRCHETIPOIndex(airRulesDir, agents, tasks, tools, workflows, modules) {
    const indexPath = path.join(airRulesDir, 'index.mdc');

    let content = `---
description: AIRchetipo - Master Index
globs:
alwaysApply: true
---

# AIRchetipo - Cursor Rules Index

This is the master index for all AIRchetipo agents, tasks, tools, and workflows available in your project.

## Installation Complete!

AIRchetipo rules have been installed to: \`.cursor/rules/air/\`

**Note:** AIRchetipo does not modify your \`.cursorrules\` file. You manage that separately.

## How to Use

- Reference specific agents: @air/{module}/agents/{agent-name}
- Reference specific tasks: @air/{module}/tasks/{task-name}
- Reference specific tools: @air/{module}/tools/{tool-name}
- Reference specific workflows: @air/{module}/workflows/{workflow-name}
- Reference entire modules: @air/{module}
- Reference this index: @air/index
    ...
    for (const module of modules) {
      ...
      if (moduleAgents.length > 0) {
        ...
        for (const agent of moduleAgents) {
          content += `- @air/${module}/agents/${agent.name} - ${agent.name}\n`;
        }
        content += '\n';
      }
      ...
      if (moduleTasks.length > 0) {
        ...
        for (const task of moduleTasks) {
          content += `- @air/${module}/tasks/${task.name} - ${task.name}\n`;
        }
        content += '\n';
      }
      ...
      if (moduleTools.length > 0) {
        ...
        for (const tool of moduleTools) {
          content += `- @air/${module}/tools/${tool.name} - ${tool.name}\n`;
        }
        content += '\n';
      }
      ...
      if (moduleWorkflows.length > 0) {
        ...
        for (const workflow of moduleWorkflows) {
          content += `- @air/${module}/workflows/${workflow.name} - ${workflow.name}\n`;
        }
        content += '\n';
      }
    }
...
- All AIRCHETIPO rules are Manual type - reference them explicitly when needed
...
AIRCHETIPO rules are configured as Manual rules (alwaysApply: false) to give you control
...
`;
  ...
  wrapLauncherWithMDC(launcherContent, metadata = {}) {
    ...
    const description = `AIRCHETIPO ${metadata.module.toUpperCase()} Agent: ${name}`;
