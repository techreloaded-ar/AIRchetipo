...
const { getAgentsFromAir, getAgentsFromDir } = require('./shared/air-artifacts');
...
class ClaudeCodeSetup extends BaseIdeSetup {
  ...
  async cleanup(projectDir) {
    ...
    if (await fs.pathExists(airCommandsDir)) {
      ...
      console.log(chalk.dim(`  Removed old AIRCHETIPO commands from ${this.name}`));
    }
  }
  ...
  async setup(projectDir, airDir, options = {}) {
    ...
    const airCommandsDir = path.join(commandsDir, 'air');

    await this.ensureDir(airCommandsDir);

    // Generate agent launchers using AgentCommandGenerator
    // This creates small launcher files that reference the actual agents in .air/
    const agentGen = new AgentCommandGenerator(this.airFolderName);
    const { artifacts: agentArtifacts, counts: agentCounts } = await agentGen.collectAgentArtifacts(airDir, options.selectedModules || []);
    ...
    const agentCount = await agentGen.writeAgentLaunchers(airCommandsDir, agentArtifacts);
    ...
    if (options.preCollectedConfig && options.preCollectedConfig._alreadyConfigured) {
      ...
      await this.processModuleInjectionsWithConfig(projectDir, airDir, options, {});
    } else if (options.preCollectedConfig) {
      await this.processModuleInjectionsWithConfig(projectDir, airDir, options, options.preCollectedConfig);
    } else {
      await this.processModuleInjections(projectDir, airDir, options);
    }
    ...
    const workflowGen = new WorkflowCommandGenerator(this.airFolderName);
    const { artifacts: workflowArtifacts } = await workflowGen.collectWorkflowArtifacts(airDir);

    // Write only workflow-command artifacts, skip workflow-launcher READMEs
    let workflowCommandCount = 0;
    for (const artifact of workflowArtifacts) {
      if (artifact.type === 'workflow-command') {
        const moduleWorkflowsDir = path.join(airCommandsDir, artifact.module, 'workflows');
    ...
    const taskToolResult = await taskToolGen.generateTaskToolCommands(projectDir, airDir);
   ...
   */
  async processModuleInjectionsWithConfig(projectDir, airDir, options, preCollectedConfig) {
   ...
   */
  async processModuleInjections(projectDir, airDir, options) {
    // Get list of installed modules
    const modules = options.selectedModules || [];
