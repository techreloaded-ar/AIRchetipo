...
class GitHubCopilotSetup extends BaseIdeSetup {
  ...
  async setup(projectDir, airDir, options = {}) {
    ...
    // Create chat mode files with air- prefix
    let modeCount = 0;
    for (const artifact of agentArtifacts) {
      const content = artifact.content;
      const chatmodeContent = await this.createChatmodeContent({ module: artifact.module, name: artifact.name }, content);

      // Use air- prefix: air-agent-{module}-{name}.chatmode.md
      const targetPath = path.join(chatmodesDir, `air-agent-${artifact.module}-${artifact.name}.chatmode.md`);
      await this.writeFile(targetPath, chatmodeContent);
      modeCount++;

      console.log(chalk.green(`  ✓ Created chat mode: air-agent-${artifact.module}-${artifact.name}`));
    }
  ...
  async configureVsCodeSettings(projectDir, options) {
    ...
    const mergedSettings = { ...airSettings, ...existingSettings };
...
`;
  ...
  /**
   * Cleanup GitHub Copilot configuration - surgically remove only AIRCHETIPO files
   */
  async cleanup(projectDir) {
    ...
    if (await fs.pathExists(chatmodesDir)) {
      ...
      for (const file of files) {
        if (file.startsWith('air-') && file.endsWith('.chatmode.md')) {
      ...
      if (removed > 0) {
        console.log(chalk.dim(`  Cleaned up ${removed} existing AIRCHETIPO chat modes`));
      }
    }
  }
}
