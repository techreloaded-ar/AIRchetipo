...
class BaseIdeSetup {
  ...
  async cleanup(projectDir) {
    ...
    if (this.configDir) {
      ...
      if (await fs.pathExists(configPath)) {
        const airRulesPath = path.join(configPath, 'air');
        if (await fs.pathExists(airRulesPath)) {
          await fs.remove(airRulesPath);
          console.log(chalk.dim(`Removed ${this.name} AIRCHETIPO configuration`));
        }
      }
    }
  }
  ...
  /**
   * Get list of agents from AIRCHETIPO installation
   * @param {string} airDir - AIRCHETIPO installation directory
   * @returns {Array} List of agent files
   */
  async getAgents(airDir) {
    const agents = [];

    // Get core agents
    const coreAgentsPath = path.join(airDir, 'core', 'agents');
  ...
  async getTasks(airDir, standaloneOnly = false) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_cfg' && entry.name !== 'agents') {
        const moduleTasksPath = path.join(airDir, entry.name, 'tasks');
  ...
  async getTools(airDir, standaloneOnly = false) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_cfg' && entry.name !== 'agents') {
        const moduleToolsPath = path.join(airDir, entry.name, 'tools');
  ...
  /**
   * Get list of workflows from AIRCHETIPO installation
   * @param {string} airDir - AIRCHETIPO installation directory
   * @param {boolean} standaloneOnly - If true, only return standalone workflows
   * @returns {Array} List of workflow files
   */
  async getWorkflows(airDir, standaloneOnly = false) {
    const workflows = [];

    // Get core workflows
    const coreWorkflowsPath = path.join(airDir, 'core', 'workflows');
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_cfg' && entry.name !== 'agents') {
        const moduleWorkflowsPath = path.join(airDir, entry.name, 'workflows');
  ...
  /**
   * Write file with content (replaces {air_folder} placeholder)
   * @param {string} filePath - File path
   * @param {string} content - File content
   */
  async writeFile(filePath, content) {
    // Replace {air_folder} placeholder if present
    if (typeof content === 'string' && content.includes('{air_folder}')) {
  ...
  /**
   * Copy file from source to destination (replaces {air_folder} placeholder in text files)
   ...
   */
  async createAgentConfig(airDir, agent) {
