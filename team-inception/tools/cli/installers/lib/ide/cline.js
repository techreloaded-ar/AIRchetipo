 ...
 * Installs AIRCHETIPO artifacts to .clinerules/workflows with flattened naming
 */
class ClineSetup extends BaseIdeSetup {
  ...
  async setup(projectDir, airDir, options = {}) {
    ...
    // Clear old AIRCHETIPO files
    await this.clearOldAIRchetipoFiles(workflowsDir);

    // Collect all artifacts
    const { artifacts, counts } = await this.collectClineArtifacts(projectDir, airDir, options);
    ...
    console.log(chalk.cyan('  AIRCHETIPO workflows are available as slash commands in Cline'));
    console.log(chalk.dim('  Usage:'));
    console.log(chalk.dim('    - Type / to see available commands'));
    console.log(chalk.dim('    - All AIRCHETIPO items start with "air-"'));
   ...
   */
  async collectClineArtifacts(projectDir, airDir, options = {}) {
  ...
  /**
   * Flatten file path to air-module-type-name.md format
   */
  ...
  /**
   * Clear old AIRCHETIPO files from the workflows directory
   */
