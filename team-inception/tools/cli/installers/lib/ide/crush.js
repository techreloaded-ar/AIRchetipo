...
class CrushSetup extends BaseIdeSetup {
  ...
  async setup(projectDir, airDir, options = {}) {
    ...
    // Create .crush/commands/air directory structure
    const crushDir = path.join(projectDir, this.configDir);
    const commandsDir = path.join(crushDir, this.commandsDir, 'air');
...
This command executes the ${toolName} tool from the AIRCHETIPO ${tool.module.toUpperCase()} module.
...
This command executes the ${workflowName} workflow from the AIRCHETIPO ${workflow.module.toUpperCase()} module.
...
`;
  ...
  async cleanup(projectDir) {
    ...
    if (await fs.pathExists(airCommandsDir)) {
      ...
      console.log(chalk.dim(`Removed AIRCHETIPO commands from Crush`));
    }
  }
}
