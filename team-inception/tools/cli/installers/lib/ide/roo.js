...
class RooSetup extends BaseIdeSetup {
  ...
  async setup(projectDir, airDir, options = {}) {
    ...
    // Generate agent launchers (though Roo will reference the actual .air agents)
    ...
    for (const artifact of agentArtifacts) {
      const slug = `air-${artifact.module}-${artifact.name}`;
      ...
      // Read the actual agent file from .air for metadata extraction
      const agentPath = path.join(airDir, artifact.module, 'agents', `${artifact.name}.md`);
      const content = await this.readFile(agentPath);

      // Create mode entry that references the actual .air agent
  ...
  async createModeEntry(agent, content, permissionChoice, projectDir) {
    ...
    // Build mode entry
    const slug = `air-${agent.module}-${agent.name}`;
  ...
  async cleanup(projectDir) {
    ...
    if (await fs.pathExists(roomodesPath)) {
      ...
      console.log(chalk.dim(`Removed ${removedCount} AIRCHETIPO modes from .roomodes`));
    }
  }
}
