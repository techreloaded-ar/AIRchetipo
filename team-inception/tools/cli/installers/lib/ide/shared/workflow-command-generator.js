...
class WorkflowCommandGenerator {
   ...
   */
  async generateWorkflowCommands(projectDir, airDir) {
    const workflows = await this.loadWorkflowManifest(airDir);
    ...
    const baseCommandsDir = path.join(projectDir, '.claude', 'commands', 'air');
    ...
    for (const workflow of standaloneWorkflows) {
      ...
      const commandContent = await this.generateCommandContent(workflow, airDir);
  ...
  async collectWorkflowArtifacts(airDir) {
    const workflows = await this.loadWorkflowManifest(airDir);
   ...
   */
  async generateCommandContent(workflow, airDir) {
    // Load the template
    const template = await fs.readFile(this.templatePath, 'utf8');

    // Convert source path to installed path
    // From: /Users/.../src/modules/aim/workflows/.../workflow.yaml
    // To: {project-root}/{air_folder}/aim/workflows/.../workflow.yaml
    ...
    if (workflowPath.includes('/src/modules/')) {
      ...
      if (match) {
        workflowPath = `${this.airFolderName}/${match[1]}`;
      }
    } else if (workflowPath.includes('/src/core/')) {
      const match = workflowPath.match(/\/src\/core\/(.+)/);
      if (match) {
        workflowPath = `${this.airFolderName}/core/${match[1]}`;
      }
    }
    ...
    return template
      ...
      .replaceAll('{{author}}', workflow.author || 'AIRCHETIPO');
  }
...
1. LOAD {project-root}/${this.airFolderName}/core/tasks/workflow.xml
...
`;
  ...
  transformWorkflowPath(workflowPath) {
    ...
    if (workflowPath.includes('/src/modules/')) {
      const match = workflowPath.match(/\/src\/modules\/(.+)/);
      if (match) {
        transformed = `{project-root}/${this.airFolderName}/${match[1]}`;
      }
    } else if (workflowPath.includes('/src/core/')) {
      const match = workflowPath.match(/\/src\/core\/(.+)/);
      if (match) {
        transformed = `{project-root}/${this.airFolderName}/core/${match[1]}`;
      }
    }
  ...
  async loadWorkflowManifest(airDir) {
    const manifestPath = path.join(airDir, '_cfg', 'workflow-manifest.csv');
