...
class TaskToolCommandGenerator {
   ...
   * @param {string} baseCommandsDir - Optional base commands directory (defaults to .claude/commands/air)
   */
  async generateTaskToolCommands(projectDir, airDir, baseCommandsDir = null) {
    const tasks = await this.loadTaskManifest(airDir);
    const tools = await this.loadToolManifest(airDir);
    ...
    const commandsDir = baseCommandsDir || path.join(projectDir, '.claude', 'commands', 'air');
...
`;
   ...
   */
  async loadTaskManifest(airDir) {
    const manifestPath = path.join(airDir, '_cfg', 'task-manifest.csv');
   ...
   */
  async loadToolManifest(airDir) {
    const manifestPath = path.join(airDir, '_cfg', 'tool-manifest.csv');
