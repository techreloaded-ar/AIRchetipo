...
/**
 * AIRchetipo CLI - Direct execution wrapper for npx
...
if (isNpxExecution) {
  ...
  const airCliPath = path.join(__dirname, 'cli', 'air-cli.js');

  if (!fs.existsSync(airCliPath)) {
    console.error('Error: Could not find air-cli.js at', airCliPath);
  ...
  try {
    ...
    execSync(`node "${airCliPath}" ${args.join(' ')}`, {
      stdio: 'inherit',
